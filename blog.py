...
# General Discussion on structure. This program implements a blog. This file is
# the best place to start to get to know the code. In this file, which is the
# controller, we define a bunch of HTTP routes that are handled by functions.
# The basic way that this magic occurs is through the decorator design pattern.
# Decorators allow you to modify a function, adding code to be executed before
# and after the function. As a side effect the bottle.py decorators also put
...
def show_post(permalink="notfound"):
    ...
    return bottle.template("entry_template",
                           dict(post=post, username=username, errors="",
                                comment=comment))
...
def post_new_comment():
    ...
    if name == "" or body == "":
        ...
        return bottle.template("entry_template",
                               dict(post=post, username=username,
...
# Displays the form allowing a user to add a new post. Only works for logged
# in users
@bottle.get('/newpost')
def get_newpost():
    cookie = bottle.request.get_cookie("session")
...
def process_login():
    ...
    if user_record:
        ...
        # the experimental version of bottle (.12).
...
# validates that the user information is valid for new signup, return True of
